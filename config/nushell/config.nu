...
$env.CARGO_HOME = "C:/Users/yasha/scoop/apps/rustup/current/.cargo"
...
def hw [] { do -i {pandoc $"($HOMED)/workspacemodules/web/classes/basiccomplex/2026.tex" -o $"($HOMED)/workspacemodules/web/classes/basiccomplex/2026.html"}; do -i {pandoc $"($HOMED)/workspacemodules/web/classes/Spivak/2026.tex" -o $"($HOMED)/workspacemodules/web/classes/Spivak/2026.html"}; cd $"($HOMED)/workspacemodules/web"; do --ignore-errors {git add .}; do --ignore-errors {git commit -m -a}; do --ignore-errors {git push origin gh-pages:gh-pages} 
} 
...
#     let branch_info = if ($branch != "") { $" (on ($branch))" } else { "" }
#     $"(ansi yellow)($env.PWD | path expand)(ansi reset)($branch_info)\n> "
# }
