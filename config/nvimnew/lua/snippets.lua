...
text({"","\\keywords{}"}),
text({"","\\begin{abstract}   "}),
text({""}), insert(1, "abstract here"), 
text({"","\\end{abstract}"}),
text({"","\\maketitledocumentclass{amsart}  "}),
...
text({"","\\DeclareMathOperator{\\coker}{coker}"}),
text({"","\\begin{document}"}),
text({"% \\href{http://yashamon.github.io/web2/papers/conformalsymplectic.pdf}{Direct link to author's version}"}),
text({"","\\title{A conformal symplectic Weinstein conjecture}"}),
text({"","\\author{Yasha Savelyev}"}),
text({"","\\thanks {This work was partially supported by PRODEP grant}"}),
text({"","\\email{yasha.savelyev@gmail.com}"}),
text({"","\\address{University of Colima, CUICBAS}"}),
text({"","\\keywords{}"}),
text({"","\\begin{abstract}   "}),
