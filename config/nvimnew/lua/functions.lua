...
Server()
local buff = vim.fn.bufname()
local commandsentence = 'te nu -c "if ( (git rev-parse --is-inside-work-tree) and (git rev-parse --git-dir) ) { git add . ; git commit -m $"$(git diff --staged)"; git push --all origin; ctags -R}"'
vim.cmd(commandsentence)
vim.cmd("buffer " .. buff)
end
