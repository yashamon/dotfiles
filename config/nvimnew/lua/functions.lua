...
Server()
local buff = vim.fn.bufname()
local commandsentence = 'te pwsh -c -nop nu -c "if ( ((git rev-parse --is-inside-work-tree) | into bool) and ((git rev-parse --git-dir) == ".git") ) { git add . ; git commit -m $(git diff --staged); git push --all origin}"'
vim.cmd(commandsentence)
vim.cmd("buffer " .. buff)
end
