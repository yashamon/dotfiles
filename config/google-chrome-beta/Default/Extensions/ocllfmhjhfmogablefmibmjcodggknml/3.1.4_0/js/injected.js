	...
	chrome.storage.local.get(null, storage => {
        ...
        if (correctCombination) {
            ...
            chrome.runtime.sendMessage({call: "getTabs"}, function(response) {
                ...
                if (tabs.length == 1) {
                    return;
                } else if (pref(storage["displayTabCylcer"], true) == false || tabs.length == 2 || (keys && (!isPartOfCombination(keys, 'ctrl') && !isPartOfCombination(keys, 'alt') && !isPartOfCombination(keys, 'meta') && !isPartOfCombination(keys, 'shift')))) {
                ...
                if (recentTabs.length) {
                    ...
                    var cssURL = chrome.runtime.getURL('main.css');
                    var css = "<style>";
